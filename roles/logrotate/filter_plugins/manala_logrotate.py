...
def config(config, exclude=[]):
    if not isinstance(config, dict):
        raise AnsibleFilterError('manala_logrotate_config expects a dict but was given a %s' % type(config))
...
def config_parameters(parameters, exclude=[]):
    if not isinstance(parameters, dict):
        raise AnsibleFilterError('manala_logrotate_config_parameters expects a dict but was given a %s' % type(parameters))
...
def config_parameter(parameters, key, required=False, default=None, comment=False):
    ...
    if required and key not in parameters:
        raise AnsibleFilterError('manala_logrotate_config_parameter requires a value for key %s' % key)
    result = ''
...
class FilterModule(object):
    ...
    def filters(self):
        filters = {
            'manala_logrotate_config': config,
            'manala_logrotate_config_parameters': config_parameters,
            'manala_logrotate_config_parameter': config_parameter,
        }
