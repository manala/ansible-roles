...
def config_section(parameters, exclude=[]):
    if not isinstance(parameters, dict):
        raise AnsibleFilterError('manala_git_config_section expects a dict but was given a %s' % type(parameters))
...
def config_parameter(parameters, key, required=False, default=None, comment=False):
    if not isinstance(parameters, dict):
        raise AnsibleFilterError('manala_git_config_parameter parameters expects a dict but was given a %s' % type(parameters))
    if not isinstance(key, string_types):
        raise AnsibleFilterError('manala_git_config_parameter key expects a string but was given a %s' % type(key))
    if required and key not in parameters:
        raise AnsibleFilterError('manala_git_config_parameter requires a value for key %s' % key)
    ...
    if value is True:
        ...
        AnsibleFilterError('manala_git_config_parameter value of an unknown type %s' % type(value))
...
class FilterModule(object):
    ...
    def filters(self):
        filters = {
            'manala_git_config': config,
            'manala_git_config_section': config_section,
            'manala_git_config_parameter': config_parameter,
        }
