...
def toml(parameters, exclude=[]):
    if not isinstance(parameters, dict):
        raise AnsibleFilterError('manala_toml expects a dict but was given a %s' % type(parameters))
    [parameters.pop(key) for key in exclude]
    return encode_toml(parameters)


def toml_parameters(parameters, exclude=[]):
    if not isinstance(parameters, dict):
        raise AnsibleFilterError('manala_toml_parameters expects a dict but was given a %s' % type(parameters))
    [parameters.pop(key) for key in exclude]
    result = ''
...
def toml_parameter(parameters, key, required=False, default=None, comment=False):
    if not isinstance(parameters, dict):
        raise AnsibleFilterError('manala_toml_parameter parameters expects a dict but was given a %s' % type(parameters))
    if not isinstance(key, string_types):
        raise AnsibleFilterError('manala_toml_parameter key expects a string but was given a %s' % type(key))
    if required and key not in parameters:
        raise AnsibleFilterError('manala_toml_parameter requires a value for key %s' % key)
    result = encode_toml({
...
class FilterModule(object):
    ...
    def filters(self):
        filters = {
            'manala_toml': toml,
            'manala_toml_parameters': toml_parameters,
            'manala_toml_parameter': toml_parameter,
        }
