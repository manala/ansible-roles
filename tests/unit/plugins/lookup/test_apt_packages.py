...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        self.maxDiff = None
        self.lookup = lookup_loader.get('manala.roles.apt_packages')
    ...
    def test_merge(self):
        self.assertListEqual([
            {'package': ['foo'], '_discriminator': 'bar', 'deb': False, 'state': 'present'},
        ], self.lookup.run([
            [
    ...
    def test_flatten(self):
        self.assertListEqual([
            {'package': ['foo', 'bar'], 'deb': False, 'state': 'present'},
        ], self.lookup.run([
            [
                {'package': 'foo'},
                [
    ...
    def test_state(self):
        self.assertListEqual([
            {'package': ['foo', 'bar'], 'deb': False, 'state': 'present'},
            {'package': ['baz'], 'deb': False, 'state': 'absent'},
        ], self.lookup.run([
            [
                {'package': 'foo'},
    ...
    def test(self):
        self.assertListEqual([
            {'package': ['foo', 'bar'], 'deb': False, 'state': 'present'},
            {'package': 'baz.deb', 'deb': True, 'state': 'present'},
        ], self.lookup.run([
            [
                'foo',
                {'package': 'bar'},
